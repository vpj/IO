...
class Call {
    ...
    handle(data: Data, packet: ResponsePacket) {
        if (this.callbacks[packet.status] == null) {
            ...
            //Handled by caller
            throw new Error(
                `No callback registered ${this.method} ${packet.status}`
            )
        }
        let self = this
        setTimeout(function() {
...
abstract class Port {
    ...
    protected abstract _respond(
        response: ResponsePacket | PacketList,
    ...
    respond(
        ...
        this._respond(
            this._createResponse(response, status, data),
            portOptions,
            callback
        )
    }

    protected shouldRespond(
        response: IOResponse,
        status: Status,
        data: any,
        portOptions: PortOptions
    ...
    protected _handleCall(
        ...
        this.handlers[packet.method](
            packet.data,
            packet,
            this.responses[packet.id]
        )
    }
    ...
    protected shouldAcceptResponse(
